...
;!function(win){
	...
	(function link(){
		...
		link.rel = 'stylesheet';
		link.href = path + 'skin/laypage.css';
		link.id = 'laypagecss';
		link.media = 'all';
	...
	Class.prototype.view = function(){
		...
		var index = config.pages > groups
			...
			,views = {
				...
				,limit: function(){
					...
					for(var index = 0, len = config.limits.length; index < len; index++){
						var item = config.limits[index];
						options.push(
							'<option value="'+ item +'"'
				...
				,skip: function(){
					return ['<span class="layui-laypage-skip">&#x5230;&#x7B2C;'
						,'<input type="number" min="1" value="'+ config.curr +'" class="layui-input">'
		...
		return ['<div class="layui-box layui-laypage layui-laypage-'+ (config.theme ? (
			...
			,function(){
				var plate = [];
				for(var index = 0, len = config.layout.length; index < len; index++){
					var item = config.layout[index];
					if(views[item]){
